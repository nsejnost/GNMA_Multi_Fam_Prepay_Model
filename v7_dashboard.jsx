...
function TabSampleLoans({ md }) {
  ...
  // Column groups for header rendering. Each entry is [key, label, format-class].
  // format-class: 'text' (left-align), 'num' (right-align), 'date' (right-align mono)
  const idCols = [
  ...
  // Loan terms (constant over the loan's life)
  const termCols = [
  ...
  // Penalty/lockout structural columns
  const penCols = [
  ...
  // Time-varying state at this loan-month
  const tvCols = [
    ['upb_M',        'UPB($M)',  'num'],
    ['age',          'Age',      'num'],
    ['mtm',          'MTM',      'num'],
    ...
    ['grf_bps',      'GrossBp',  'num'],
    ['ppp',          'PPP',      'num'],
    ['net_refi_bps', 'NetBp',    'num'],
  ];
  const summaryCols = [
    ['pred_cpr',      'Pred CPR%', 'num'],
    ['actual_prepay', 'Actual',    'num'],
  ];
  ...
  return (
    ...
    >
      ...
      {/* Group color hints in header band */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 4, color: T.textDim,
                    fontSize: 10, fontFamily: FONT_MONO }}>
        <span style={{ background: T.panelLight, padding: '2px 6px', borderRadius: 3 }}>identifiers</span>
        <span style={{ background: '#0E3A5F', padding: '2px 6px', borderRadius: 3 }}>loan terms</span>
        <span style={{ background: '#5C2A1A', padding: '2px 6px', borderRadius: 3 }}>penalty / lockout</span>
        <span style={{ background: '#1F4D2A', padding: '2px 6px', borderRadius: 3 }}>state at loan-month</span>
        <span style={{ background: '#92400E', padding: '2px 6px', borderRadius: 3 }}>prediction</span>
        <span style={{ background: '#3F0F4F', padding: '2px 6px', borderRadius: 3 }}>attribution (CPR pp)</span>
      </div>
      ...
      <div style={{ overflowX: 'auto', maxHeight: 620, overflowY: 'auto',
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse',
          ...
          <thead>
            <tr>
              {[
                {cols: idCols,      bg: T.panelLight},
                {cols: termCols,    bg: '#0E3A5F'},
                {cols: penCols,     bg: '#5C2A1A'},
                {cols: tvCols,      bg: '#1F4D2A'},
                {cols: summaryCols, bg: '#92400E'},
              ].map(grp => grp.cols.map(([k, lab, fmt]) => (
                <th key={k}
                    style={{ ...headTd, background: grp.bg,
          ...
          <tbody>
            {sorted.map((row, i) => {
              const renderCell = ([k, _lab, fmt]) => {
                ...
                // Highlight pred_cpr + actual specially
                if (k === 'pred_cpr') {
                  return <td key={k} style={{ ...cellStyle, color: T.accent, fontWeight: 600 }}>
                    {row.pred_cpr.toFixed(2)}
                  </td>;
                }
                if (k === 'actual_prepay') {
                  return <td key={k} style={{ ...cellStyle, textAlign: 'center',
                    color: row.actual_prepay ? T.green : T.textDim }}>
                    {row.actual_prepay ? '✓' : '·'}
                  </td>;
                }
                if (k === 'loan_id') {
                  return <td key={k} style={{ ...cellStyle, color: T.text }}>{display}</td>;
                }
                return <td key={k} style={cellStyle}>{display}</td>;
              };
              return (
                <tr key={i} style={{
                  ...
                  {idCols.map(renderCell)}
                  {termCols.map(renderCell)}
                  {penCols.map(renderCell)}
                  {tvCols.map(renderCell)}
                  {summaryCols.map(renderCell)}
                  {multNames.map(name => {
